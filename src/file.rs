...
pub fn open_file(filename: &str) -> Result<String, io::Error> {
    ...
    if path.exists() {
        ...
        if !File::open(path).is_ok() {
            let err_message =
                format!("Error: File is not readable.\nHint: Try to do `$ chmod +r {filename}`");
            return Err(io::Error::new(
        ...
        if !can_write(&path) {
            let err_message =
                format!("Error: File is not writeable.\nHint: Try to do `$ chmod +w {filename}`");
            return Err(io::Error::new(
                io::ErrorKind::PermissionDenied,
                err_message.red().to_string(),
            ));
        ...
        let content = fs::read_to_string(filename).map_err(|err| {
            io::Error::new(
                err.kind(),
                format!("couldn't read {filename:?}: {err}")
                    .red()
                    .to_string(),
            )
        })?;
...
/// This function writes the given content to the file at `path`.
pub fn save_file(path: &Path, content: &String) -> Result<(), io::Error> {
